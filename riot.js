...
(function () {
    ...
    (function (riot, evt) {
        ...
        if (!this.top)
            return;
        var loc = location, fns = riot.observable(), win = window, current;
        function hash() {
...
*/
    var brackets = function (orig, s, b) {
        return function (x) {
            ...
            // else, get brackets
            return x && x.test ? s == orig ? x : RegExp(x.source.replace(/\{/g, b[0].replace(/(?=.)/g, '\\')).replace(/\}/g, b[1].replace(/(?=.)/g, '\\')), x.global ? 'g' : '') : b[x];
        };
    }('{ }');
    var tmpl = function () {
        var cache = {}, re_vars = /(['"\/]).*?[^\\]\1|\.\w*|\w*:|\b(?:(?:new|typeof|in|instanceof) |(?:this|true|false|null|undefined)\b|function *\()|([a-z_$]\w*)/gi;
        // [ 1               ][ 2  ][ 3 ][ 4                                                                                  ][ 5       ]
        // find variable names:
...
.replace(/\n/g, ' ')    // trim whitespace, curly brackets, strip comments
.replace(brackets(/^[{ ]+|[ }]+$|\/\*.+?\*\//g), '');
 ...
 ? '[' + s.replace(/\W*([\w- ]+)\W*:([^,]+)/g, function (_, k, v) {
            ...
            }) + '].join(" ").trim()'    // if js expression, evaluate as javascript
 : wrap(s, n);
        }
...
+ (s.replace(re_vars, function (s, _, v) {
                return v ? '(d.' + v + '===undefined?window.' + v + ':d.' + v + ')' : s;
            })    // break the expression if its empty (resulting in undefined value)
|| 'x') + '}finally{return '    // default to empty string for falsy values except zero
+ (nonull === true ? '!v&&v!==0?"":v' : 'v') + '}}).call(d)';
    ...
    function _each(dom, parent, expr) {
        ...
        parent.one('update', function () {
            ...
            // mount new / reorder
            var nodes = root.childNodes, prev_index = [].indexOf.call(nodes, prev);
    ...
    function Tag(impl, conf) {
        var self = riot.observable(this), opts = inherit(conf.opts) || {}, dom = mkdom(impl.tmpl), parent = conf.parent, expressions = [], child_tags = [], root = conf.root, item = conf.item, fn = impl.fn, attr = {}, loop_dom;
        if (fn && root.riot)
            return;
        root.riot = true;
        extend(this, {
...
;
        ...
        this.unmount = function () {
            var el = fn ? root : loop_dom, p = el.parentNode;
            if (p) {
                if (parent)
                    p.removeChild(el);
                else
    ...
    function update(expressions, tag, item) {
        each(expressions, function (expr) {
            var dom = expr.dom, attr_name = expr.attr, value = tmpl(expr.expr, tag), parent = expr.dom.parentNode;
            if (value == null)
                value = '';
            ...
            if (typeof value == 'function') {
                setEventHandler(attr_name, value, dom, tag, item)    // if- conditional
;
...
;
    ...
    function mkdom(template) {
        var tag_name = template.trim().slice(1, 3).toLowerCase(), root_tag = /td|th/.test(tag_name) ? 'tr' : tag_name == 'tr' ? 'tbody' : 'div', el = document.createElement(root_tag);
        el.stub = true;
...
*/
    ...
    riot.mount = function (selector, tagName, opts) {
        ...
        var tags = [];
        function push(root) {
            var name = tagName || root.tagName.toLowerCase(), tag = mountTo(root, name, opts);
            if (tag)
                tags.push(tag);
        }
        // DOM node
